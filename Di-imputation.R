...
#qplot(df[,6], df[,22], data=df, colour=cl) + theme_bw() + theme(aspect.ratio=1)
#####explorotaory 
df.ex <- df %>%
  group_by(cl) %>%
...
ggparcoord(df, columns=3:17, groupColumn=19,scale="globalminmax", boxplot=TRUE, 
           showPoints=FALSE, alphaLines=0.05) + 
  xlab("") + ylab("PctCorrect") + theme_bw()
